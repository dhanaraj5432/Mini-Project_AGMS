...
if(isset($_POST["submit"]))
{
    if(isset($_POST["radio"]))
    {
        ...
        $clickedradio=$_POST["radio"];
        $sql = "SELECT * from $clickedradio where User='$username' and Password='$password'";
        $result = mysqli_query($conn, $sql);
        if (mysqli_num_rows($result) > 0) 
        {
            session_start();
            $_SESSION["username"]=$_POST["username"];
            header("Location:".$clickedradio.".php");
        } 
        else 
        {
          echo "no such user";
...
<html>
    ...
    <body>
        <form method="post" action="#">
            ...
            <div id="rbt">
                <input type="radio" id="rdbt1" name="radio" value="User">User
                <input type="radio" id="rdbt1" name="radio" value="Worker">Worker
                <input type="radio" id="rdbt1" name="radio" value="Admin">Admin
            </div>
        </form>
    </body>
</html>